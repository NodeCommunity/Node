...
The Node Core project operates an open contributor model where anyone is
...
The following applies to code changes to the Node Core project, and is not to be
confused with overall Node Network Protocol consensus changes.

Whether a pull request is merged into Node Core rests with the project merge
...
Patches that change Node consensus rules are considerably more involved than
...
Where a patch set proposes to change the Node consensus, it must have been
...
The project leader is the release manager for each Node Core release.
